...
#define VDEV_COMMON_MEMBERS \
	...
	void(*leftbtndown)(void *ctxt, int x, int y);						\
	void(*leftbtnup)(void *ctxt);										\
	void(*mousemove)(void *ctxt, int x, int y);							\
	void(*mousewheel)(void *ctxt, short delta);							\
	void(*setrendertype)(void *ctxt, int type);							\
...
// 函数声明
void* vdev_create  (int type, void *app, int bufnum, int w, int h, int frate, char *temp);
