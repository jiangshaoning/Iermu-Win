...
static int player_prepare(PLAYER *player)
{
	...
	av_dict_set(&format_opts, "probesize", "200*1024", 0);
	av_dict_set(&format_opts, "preset", "superfast", 0);
	av_dict_set(&format_opts, "buffsize", "1", 0);
	av_dict_set(&format_opts, "stimeout", "10000000", 0);
	av_dict_set(&codec_opts, "analyzemaxduration", "0", 0);
    ...
    // find stream info
	if (avformat_find_stream_info(player->avformat_context, &codec_opts) < 0) {
    ...
    player->render = render_open(
        ...
        player->init_params.video_owidth, player->init_params.video_oheight, player->init_params.eapil_template);
...
void player_leftbtndown(void *hplayer, int x, int y)
{
    if (!hplayer) return;
    PLAYER *player = (PLAYER*)hplayer;
    render_leftbtndown(player->render, x, y);
}
...
void player_leftbtnup(void *hplayer)
{
    if (!hplayer) return;
    PLAYER *player = (PLAYER*)hplayer;
    render_leftbtnup(player->render);
}
...
void player_mousemove(void *hplayer, int x, int y)
{
    if (!hplayer) return;
    PLAYER *player = (PLAYER*)hplayer;
    render_mousemove(player->render, x, y);
}
...
void player_mousewheel(void *hplayer, short delta)
{
    if (!hplayer) return;
    PLAYER *player = (PLAYER*)hplayer;
    render_mousewheel(player->render, delta);
}
...
void player_setrendertype(void *hplayer, int type)
{
    if (!hplayer) return;
    PLAYER *player = (PLAYER*)hplayer;
    render_setrendertype(player->render, type);
}
...
int player_getstatus(void *hplayer)
{
	if (!hplayer) return 0;
	PLAYER *player = (PLAYER*)hplayer;
