...
void* render_open(int adevtype, int srate, AVSampleFormat sndfmt, int64_t ch_layout,
                  int vdevtype, void *surface, AVRational frate, AVPixelFormat pixfmt, int w, int h, char *temp)
{
    ...
    render->vdev = vdev_create(vdevtype, surface, 0, w, h, (int)((double)frate.num / frate.den + 0.5), temp);
...
void render_video(void *hrender, AVFrame *video)
{
    ...
    do {
		...
		VDEV_COMMON_CTXT * vct = (VDEV_COMMON_CTXT*)render->vdev;
		if (vct->rendertype == VDEV_RENDER_TYPE_EAPIL)
		{
			vct->w = render->video_width;
			vct->h = render->video_height;
		}
        ...
        if (video->format == AV_PIX_FMT_DXVA2_VLD) {
            ...
            if (picture.data[0] && video->pts != -1) {
				if (vct->rendertype == VDEV_RENDER_TYPE_EAPIL)
				{
					...
					render->sws_context = sws_getCachedContext(render->sws_context,
						render->video_width, render->video_height, (AVPixelFormat)video->format, render->video_width, render->video_height,
						AV_PIX_FMT_YUV420P, sws_flags, NULL, NULL, NULL);

					picture.data[0] = picture.data[0];
					picture.data[1] = picture.data[0] + render->video_width*render->video_height;
					picture.data[2] = picture.data[0] + render->video_width*render->video_height * 5 / 4;

					picture.linesize[0] = render->video_width;
					picture.linesize[1] = render->video_width / 2;
					picture.linesize[2] = render->video_width / 2;
				}
...
void render_leftbtndown(void *hrender, int x, int y)
{
    if (!hrender) return;
    RENDER *render = (RENDER*)hrender;
	vdev_leftbtndown(render->vdev, x, y);
}

void render_leftbtnup(void *hrender)
{
    if (!hrender) return;
    RENDER *render = (RENDER*)hrender;
	vdev_leftbtnup(render->vdev);
}

void render_mousemove(void *hrender, int x, int y)
{
    if (!hrender) return;
    RENDER *render = (RENDER*)hrender;
	vdev_mousemove(render->vdev, x, y);
}


void render_mousewheel(void *hrender, short delta)
{
    if (!hrender) return;
    RENDER *render = (RENDER*)hrender;
	vdev_mousewheel(render->vdev, delta);
}

void render_setrendertype(void *hrender, int type)
{
    if (!hrender) return;
    RENDER *render = (RENDER*)hrender;
	vdev_setrendertype(render->vdev, type);
}
